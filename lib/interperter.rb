# requires for interperter/

require_relative '../common/code_container'
require_relative 'interperter/register'

require_relative 'interperter/default_handler'
require_relative 'interperter/interrupt_called'
#require_relative 'interperter/error_state'
#require_relative 'interperter/stack_not_empty'
require_relative 'interperter/opcodes'
require_relative 'interperter/code_interperter'
require_relative 'interperter/break_point_reached'

