...
class VasmTransform < Parslet::Transform
  ...
  rule(label: simple(:label), opcode: simple(:opcode), operand: simple(:operand)) { Label.new(label, opcode, operand.to_s) }
  ...
  rule(opcode: simple(:opcode), operand: simple(:operand)) { Opcode.new(opcode, operand.to_s) }
